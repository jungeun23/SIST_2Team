...
//		PATH = "data\\schedule\\meetingRoom.txt";
	...
	public static void makeScedule() {
		try {
			...
			String[] ment2 = { "신입사원 인성 교육", "신입사원 직무 교육", "신입사원 멘토링", "산업 안전 보건 교육", "직장 내 성희롱 예방 교육", "개인정보보호 교육",
					"엑셀실무", "기초 재무회계", "데이터 분석과 통계", "한 장 보고서 작성" };

			for (int i = 0; i < 300; i++) {

				int nh = rnd.nextInt(hr.size());
				int nm = rnd.nextInt(5) + 1;
				...
				String title = DummyMake.getTitleDummy();
				String content = DummyMake.getTitleDummy();
				line += String.format("%s,%s,%s,%s\n", hr.get(nh)[0], ("2021-" + nm + "-" + nd), title, content);
				BufferedWriter writer = new BufferedWriter(new FileWriter("data\\schedule\\schedule.txt"));
				writer.write(line);
				writer.close();
			}
		} catch (Exception e) {
...
//			// 교육센터 더미 만들기
			...
			for (int i = 0; i < 300; i++) {
				...
				String title = DummyMake.getTitleDummy();
				String content = DummyMake.getTitleDummy();
				line += String.format("%s,%s,%s,%s,%s,%s\n", hr.get(nh)[0], hr.get(nh)[1], hr.get(nh)[2],
						("2021-" + nm + "-" + nd), ment1[nn], ment2[ne]);
